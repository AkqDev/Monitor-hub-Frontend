...
import { 
  FaTasks, FaPlus, FaTrash, FaCheck, FaPlay, FaSpinner 
} from "react-icons/fa";
import { 
  MdAssignment, MdAdd, MdDelete, MdCheckCircle, MdPlayArrow, MdLoop 
} from "react-icons/md";
...
export default function TaskManager({ token }) {
  ...
  const handleCreateTask = async (values) => {
    ...
    try {
      ...
      setTasks((prev) => [
        { ...res.data.task, key: res.data.task._id },
        ...prev,
      ]);
  ...
  const getStatusTag = (status) => {
    const mapping = {
      "in-progress": {
        color: "green",
        icon: <FaPlay className="w-4 h-4 text-blue-400" />,
        text: "In Progress",
      },
      completed: {
        color: "green",
    ...
    return (
      <Tag
        color={s.color}
        className="!flex !items-center gap-1 font-semibold !py-1"
      >
        {s.icon} {s.text}
      </Tag>
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "assignee",
      render: (text) => (
        <Tag className="bg-black text-white font-semibold">{text}</Tag>
      ),
      ...
      render: (_, record) => (
        ...
        >
          <Button
            ...
            size="small"
            className="!bg-green-500 !font-semibold !border-none font-[poppins]"
          >
            Delete
          </Button>
        </Popconfirm>
    ...
    <div className="!bg-black/25 rounded-2xl p-6 !border-0 font-inter">
      <div className="flex justify-between items-center mb-4 border-b border-white/20 pb-3">
        <h2 className="text-xl font-semibold flex items-center gap-2 !text-green-500 font-[poppins]">
          <FaTasks className="w-6 h-6 text-green-400" /> Task Management
        </h2>
        ...
        <Button
          ...
          className="flex items-center gap-1 !text-gray-100 !bg-green-500 !border-0 rounded-full font-[poppins] !px-6 !py-3 !font-semibold "
        >
          Assign New Task
        </Button>
      </div>
      ...
      <Spin spinning={loading}>
        <Table
          ...
          rowClassName={() => "bg-black/5"}
          onRow={() => ({
            style: { cursor: "default", background: "black/25" },
          })}
        />
      </Spin>
      ...
      <Modal
        title={
          <span className="font-[poppins] text-black font-semibold">
            Assign a New Task
          </span>
        }
        open={isModalVisible}
        ...
        >
          ...
          <Form.Item>
            <Button
              ...
              htmlType="submit"
              loading={loading}
              className="w-full !bg-green-600 hover:!bg-green-700 border-none !font-bold"
            >
              Assign Task
            </Button>
          </Form.Item>
